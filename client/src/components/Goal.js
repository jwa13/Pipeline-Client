...
const Goal = ({ goals }) => {
    const foramtDate = (unformattedDate) => {
                ...
                {goals.map((item, index) => {
                    return (
                        <div key={index} className="shadow-md p-2 grid grid-cols-3 pb-2 ml-1">
                            ...
                            <h2 className="text-gray-700 font-bebas-neue flex justify-center">Created On: <span className="pl-1" style={{ fontFamily: 'Arial' }}>{foramtDate(item.dateCreated)}</span></h2>
