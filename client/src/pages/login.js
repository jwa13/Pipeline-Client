...
export default function login() {
    ...
    function SubmitButton() {
        if(email && password) {
            return <button onClick={submit} className="w-3/4 px-6 py-4 m-6 bg-[hsl(43.3,46,64)] text-black font-bebas-neue text-2xl rounded-none font-bold text-white hover:bg-gray-400 transition border-[3px] border-solid border-white">Login</button>
        } else {
            return <button onClick={submit} disabled className="w-3/4 px-6 py-4 m-6 bg-[hsl(43.3,46,64)] text-black font-bebas-neue text-2xl rounded-none font-bold text-white hover:bg-gray-400 transition border-[3px] border-solid border-white">Login</button>
        }
    }
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-white">
            <div className={`relative transition-all duration-300 ease-in-out ${loginMessage ? 'pt-20' : 'pt-0'} flex justify-center`}>
                ...
                <div className="bg-[url('/astro-bg-2.png')] rounded-2xl border-[5px] border-solid border-gray-500 shadow-lg p-8 w-96 h-[500px] flex flex-col justify-center">
                    ...
                    <div className="space-y-4 pt-2 pb-6 flex flex-col items-center">
                        <form onSubmit={submit} className="flex flex-col items-center w-full">
                            {/* <label htmlFor="email" className="text-white font-bebas-neue text-m mr-[235px]">Your Email:</label> */}
                            <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-5 mb-5 ml-2 mr-2 py-1 pl-2 w-full text-black text-l border-[3px] border-solid border-gray-500"></input>
                            {/* <label htmlFor="password" className="text-white font-bebas-neue text-m mr-[210px]">Your Password:</label> */}
